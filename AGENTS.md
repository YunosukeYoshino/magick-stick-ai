...
### コーディングルール
#### 禁止事項
- **TypeScript any/unknown型の使用禁止**: 型安全性を確保するため、`any`および`unknown`型の使用を禁止します
- **コミット時の--no-verifyオプション禁止**: pre-commit hookをバイパスしないようにします

#### 型定義のベストプラクティス
- 可能な限り具体的な型を使用してください
- ユニオン型 (`string | number`) を活用して型の安全性を高めてください
- インターフェースや型エイリアスを適切に定義してください
- ジェネリック型を活用して再利用性を高めてください
